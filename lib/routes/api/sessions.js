...
module.exports = store => {
  ...
  router.post('/', helper.checkAuth(true), bodyParser, (req, res, next) => {
    ...
    const now = moment()
    logger.enabledLevels.debug && log.debug('req.body :', req.body)
    store.getQuiz(req.body && req.body.quizId)
      ...
      .then(session => {
        logger.enabledLevels.debug && log.debug('session :', session)
        ...
        try {
          ...
          logger.enabledLevels.warn && log.warn('Error :', err)
          return session
        }
