...
class MemoryStore extends Store {
  ...
  saveQuiz(quiz) {
    return Promise.resolve()
      .then(() => {
        ...
        _.set(this.data, ['quizzes', quiz.id], quiz)
        return quiz
      })
  }
  ...
  getSessions() {
    return Promise.resolve()
      .then(() => {
        ...
        return helper.clone(this.data.sessions)
      })
  }
  ...
  deleteSession(email, quizId) {
    return this.findSessions(email, quizId)
      .then(session => {
        const predicate = {user: {email}}
        if (quizId) {
          predicate.quizId = quizId
        }
        return _.remove(this.data.sessions, predicate)
      })
  }
