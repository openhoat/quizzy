...
class RedisStore extends Store {
  ...
  init() {
    this.redis = RedisCli(_.get(this, 'config.redis.cli'))
    super.init()
  }
  ...
  saveUser(user) {
    ...
    return Promise.resolve()
      .then(() => {
        ...
        const key = this.getKey('users', user.email)
        return this.redis.exec('set', key, JSON.stringify(user))
      })
      .return(user)
  }

  getUser(email) {
    const key = this.getKey('users', email)
    return Promise.resolve()
      .then(() => {
        if (!this.started) {
          throw new Store.errors.NotStartedError()
